...
class GamePictureViewSet(ViewSet):
    ...
    def create(self, request):
        ...
        format, imgstr = request.data["acion_pic"].split(';base64,')
        ext = format.split('/')[-1]
        data = ContentFile(base64.b64decode(imgstr), name=f'{request.data["game"]}-{uuid.uuid64()}.{ext}')
        ...
        try:
            game_picture.save()
            serializer = GamePictureSerializer(game_picture, contet={'request': request})
